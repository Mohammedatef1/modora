...
const Input = <TFieldValue extends FieldValues>({label, type = "text", register, name, error} : InputProps<TFieldValue> )  => {
  return (
    <div className="flex flex-col gap-y-px">
      ...
      <input type={type} className={`px-3 py-1 text-sm md:text-base border rounded-sm bg-gray-50 ${error ? 'border-red-500 outline-red-500' : 'border-gray-300'}`} {...register(name)} />
