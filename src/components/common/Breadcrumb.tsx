  ...
  return (
    <div className="relative h-56 md:h-72 lg:h-80 flex flex-col items-center justify-center text-center overflow-hidden">
      ...
      <div className="relative z-5 text-center">
        <h3 className="font-semibold text-2xl md:text-4xl lg:text-5xl mb-3">{pathItems[pathItems.length -1]}</h3>
        <div className="flex items-center justify-center gap-x-1.5">
          {pathItems.map((item, index) => (
            <>
              {pathItems.length === index + 1 ? <p>{capitalizeFirstLetter(item)}</p> : <Link to={item === 'home' ? '/' :  item}>{capitalizeFirstLetter(item)}</Link> }
